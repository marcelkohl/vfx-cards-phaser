/**
 * Dual traveling rim — same SDF path as the single shader rim,
 * with a second head locked at +0.5 perimeter offset.
 */
export const CARD_RIM_LINE_DOUBLE_FRAGMENT_SHADER = `
...
void main() {
    ...
    float along01 = distanceAlongRim(p, halfSize, radius) / max(peri, 1e-4);

    float headA = fract(uProgress);
    float headB = fract(uProgress + uOppositeOffset);
    float mask = max(
        segmentIntensity(along01, headA, uSegment),
        segmentIntensity(along01, headB, uSegment)
    ) * uIntensity;
