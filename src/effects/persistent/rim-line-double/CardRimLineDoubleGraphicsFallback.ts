...
export type { RimLineDoubleOptions } from './rimLineDoubleOptions'
...
export class CardRimLineDoubleGraphicsFallback implements PersistentEffect {
  ...
  private rebuild(context: EffectContext, preserveProgress: boolean): void {
    ...
    this.samplePoints = Array.from({ length: 33 }, () => new Phaser.Math.Vector2())
  ...
  private buildPath(width: number, height: number): void {
    ...
    const { outerPadding, cornerRadius: radius } = this.options
  ...
  private drawSegments(): void {
    ...
    graphics.clear()
    this.strokeHead(this.progress)
    this.strokeHead(wrap01(this.progress + options.oppositeOffset))
  }

  private strokeHead(headProgress: number): void {
    const graphics = this.graphics
    const options = this.options
    ...
    const sampleCount = this.samplePoints.length - 1
    const segmentLength = this.perimeter * options.segmentLength
    const tailDistance = headProgress * this.perimeter - segmentLength
    const lineWidth = Math.max(options.coreWidth * 8, 6)
    const layers = [
    ...
    for (let i = 0; i <= sampleCount; i += 1) {
      const distance = tailDistance + (segmentLength * i) / sampleCount
    ...
    for (const layer of layers) {
      for (let i = 0; i < sampleCount; i += 1) {
        ...
        graphics.lineStyle(
          lineWidth * layer.widthScale * (0.6 + 0.4 * intensity),
          layer.color,
          layer.alphaScale * intensity,
        )
        graphics.beginPath()
...
function intensityAt(t: number): number {
  const tail = Math.pow(Phaser.Math.Clamp(t, 0, 1), 0.8)
  const head = Phaser.Math.Clamp((1 - t) / 0.12, 0, 1)
