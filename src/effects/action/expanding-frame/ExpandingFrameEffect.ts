...
import { EXPANDING_FRAME_FRAGMENT_SHADER } from './expandingFrame.frag'
import {
  resolveExpandingFrameOptions,
  sampleExpandingFrameEnvelope,
  type ExpandingFrameOptions,
  type ResolvedExpandingFrameOptions,
} from './expandingFrameOptions'

export type {
  ExpandingFrameOptions,
  ExpandingFramePosition,
} from './expandingFrameOptions'
export { EXPANDING_FRAME_DEFAULTS } from './expandingFrameOptions'

export type ExpandingFrameFinishCallback = (
  effect: ExpandingFrameEffect,
) => void
...
/**
 * Temporary luminous frame that starts near the target bounds and expands
 * outward once per `run()` (Action Effect). Optional translucent interior fill
 * (`fillIntensity`) washes the enclosed area while the boundary stays denser.
 * Opacity fades while scale continues — releasing energy wave.
 * Idle after `enable()` — call `run()` to play one expansion.
 */
export class ExpandingFrameEffect implements ActionEffect {
  public readonly id = EFFECT_IDS.expandingFrame
  public readonly name = 'Expanding Frame'
  public readonly description =
    'Moldura luminosa que expande para fora; use run() / onFinish().'
  public readonly kind = 'action' as const

  private inputOptions: ExpandingFrameOptions
  private options: ResolvedExpandingFrameOptions | null = null
  ...
  public reconfigure(
    ...
    this.inputOptions = {
      ...this.inputOptions,
      ...(options as ExpandingFrameOptions),
    }
    this.rebuild(context)
    if (wasRunning) {
      this.run()
  ...
  /** Starts (or restarts) one outward expansion. Fires `onFinish` when it ends. */
  public run(): this {
  ...
  public onFinish(callback: (effect: ActionEffect) => void): () => void {
    const listener = callback as ExpandingFrameFinishCallback
  ...
  private rebuild(context: EffectContext): void {
    this.clearVisuals()
    this.options = resolveExpandingFrameOptions(this.inputOptions)
    this.running = false
    this.elapsedMs = 0
    this.strength = 0
    this.scale = this.options.startScale
  ...
  private buildShader(context: EffectContext): void {
    ...
    this.shader = context.scene.add.shader(
      {
        name: `ExpandingFrameShader-${instanceId}`,
        shaderName: 'ExpandingFrameShader',
        fragmentSource: EXPANDING_FRAME_FRAGMENT_SHADER,
        setupUniforms: (setUniform: (name: string, value: unknown) => void) => {
          ...
          setUniform('uFillIntensity', fillIntensity)
          setUniform(
            'uOpacity',
  ...
  /**
   * Canvas / non-WebGL path — optional translucent fill under a soft
   * corner-weighted rim; alpha + scale driven by the expansion envelope.
   */
  private buildGraphicsFallback(context: EffectContext): void {
    ...
    // Interior luminous plate — weaker than the boundary strokes below.
    if (fillIntensity > 0.004) {
      const fillAlpha = Math.min(
        opacity * Math.min(intensity, 1.6) * fillIntensity * 0.42,
        0.85,
      )
      if (fillAlpha >= 0.02) {
        ...
        this.graphics.fillRoundedRect(
          ...
          pathWidth,
          pathHeight,
          Math.min(cornerRadius, minSide / 2),
        )
      }
    }
